import { PresignedUrlResponse } from "@/app/api/s3/presigned/one/route";
import { PresignedUrlProp, ShortFileProp } from "./types";

export const MAX_FILE_SIZE_NEXTJS_ROUTE = 4; // 4 mb
export const MAX_FILE_SIZE_S3_ENDPOINT = 100;
export const FILE_NUMBER_LIMIT = 10;

/**
 * Gets presigned url for uploading file to S3
 * @param formData form data with file to upload
 * @returns
 */
export const getPresignedUrl = async (file: ShortFileProp) => {
  const response = await fetch("/api/s3/presigned/one", {
    method: "POST",
    headers: {
...
export function validateFiles(
  ...
  if (files.length > FILE_NUMBER_LIMIT) {
    return `Вы можете загрузить не более ${FILE_NUMBER_LIMIT} файлов за 1 раз`;
  }
  return;
}
